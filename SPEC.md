...
贾维斯提供 `list_approval_rules` / `remove_approval_rule`，面板设置支持列表和精确删除。**默认仍逐次审批；只有用户启用 P1 的对应模式才使用预设。** 高危操作不能创建预设：包括危险命令、工作区外写入、受保护配置目录；未知 shell 语法/选项、含 `..` 的路径、悬空或越界符号链接、全局安装也保留逐次审批。

### Phase-2（P1 已实现，默认关闭）：四级审批

| 级 | off | safe | safe+grey |
|---|---|---|---|
| 安全 | 转用户 | 只读白名单自动批准 | 只读白名单自动批准 |
| 灰色 | 转用户 | 转用户 | 模型 allow 且 confidence ≥ 0.9 才批准；deny 附原因转用户 |
| 中危 | 转用户 | 转用户 | 命中工作区预设，或至少两次用户批准且无拒绝时批准；其余转用户 |
| 高危 | 转用户 | 转用户 | 转用户，即使伪造预设或存在历史也不自动批准 |

`autoApprove` 在 DSH 贾维斯设置页配置，默认 `off`，下一次请求生效；没有设置服务时使用插件配置，默认行为不变。`approval-tier.ts` 的规则表独立于 DSH；读取、搜索、列目录及受限只读 shell 形式为 safe，写文件/本地安装/普通 push 等为 medium，其余可判定的简单操作为 grey。危险命令、越界/受保护路径写入及无法可靠解析的混合 shell 为 high；路径先检查组件再解析符号链接，拒绝含 `..`、悬空链接和 cwd 覆盖。pip 安装只有显式指定工作区内 `--target` 才可判 medium，其余保守转用户。

历史读取使用 approvals store 同一读锁快照，按完整 cwd、tool 和 J3 fingerprint 限定；正向学习另要求原始 command/args 完全相同以防指纹碰撞，同指纹任一用户拒绝即否决学习。自动批准不计作用户样本；损坏或读取失败不产生放行依据。灰色模型复用 oneShot 和判断模型/超时配置，输入完整操作、任务上下文及最近同指纹记录；整个准备和推理过程有超时，非法输出、低置信、服务失败及取消均回落原审批竞答。放行前重新检查设置、请求与任务身份、操作和分类。

每次自动批准先返回 `allowed-once`，再异步写 J3 HTML（`source="jarvis"`、实际 tier 和原因）；写失败不影响结果。灰色批准播报一句，安全项静默。面板悬停显示本次插件运行中最近五条自动批准；只有依据 P0 预设的条目显示“撤销此规则”，精确删除预设并移除相关入口，不撤回已执行操作。高危永不自动批准，所有等级均不自动拒绝（D6）。
...
**当前已落地**：插件创建/恢复与标题固定、托管集合、任务台账、轮末判断与用户批准续做、完成播报合并和提问排队、审批竞答与异步 HTML 记录、工作区审批预设 P0 与默认关闭的分级自动审批 P1、voice-mini 轮末让出、原生面板及任务进度。

**记忆基础能力 M1/M2/M3/M6 已落地。后续按 backlog 推进**：稳定性 W1/O1/T1 与真机验收 Q1；语音输入 S0/S1。分类器/固化 M4/M5 暂缓，L3 摘要 M7 和多化身 A1 条件触发。唤醒词、跨平台移植不在当前范围。
