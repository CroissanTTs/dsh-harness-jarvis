...
| M4 | 处置分类器（pass B） | M2、M3、J2 的 LLM 助手 | 大 | 待办 |
| M5 | 固化 / 长期清理 / 归档（pass C、E、F） | M4 | 大 | 待办 |
...
- **按判断结果处理**：
  - `satisfied` → `deps.say('{会话名}：{summary}')`，任务标 `done`。**不唤醒贾维斯**（省一轮模型调用）。⚠ 见决策 D2。
...
- **推荐方案**（⚠ 决策 D1）：贾维斯接管"交给他的任务"的轮末播报，voice-mini 让出。
...
| 编号 | 问题 | 推荐默认 | 状态 |
|---|---|---|---|
| D1 | 托管会话的轮末播报由谁说？（voice-mini 已经会总结每个会话的结果） | 有贾维斯任务的托管会话由贾维斯说，voice-mini 让出（J5）；其他会话照旧由 voice-mini 说 | ⚠ 待用户拍板 |
| D2 | 判断为"满足"时，是插件直接播报，还是唤醒贾维斯让它说？ | 插件直接播报（不花贾维斯一轮模型调用）；只有"不满足"才唤醒贾维斯 | ⚠ 待用户拍板 |
| D3 | 托管会话要不要在 DSH 标题里加"贾维斯-"前缀？ | 不加。`rename` 会钉住标题、让 DSH 不再自动命名，破坏用户自己的标题；面板里已经把托管会话单独列出，足够区分。配置项 `titlePrefix` 保留给以后"贾维斯自己创建的会话" | ⚠ 待用户拍板 |
| D4 | 记忆系统做到哪一步？ | 先做 M1–M3 + M6（能记、能查、自动留底），M4/M5 等用一段时间再定 | ⚠ 待用户拍板 |
