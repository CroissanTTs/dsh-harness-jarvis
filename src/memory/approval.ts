...
export function approvalMemory(html: string): ApprovalMemory | undefined {
  try {
    ...
    const summary = `${decision.allow === 'true' ? '批准' : '拒绝'} ${operation.tool}: ${operation.command}`;
    return { tag: '审批', key: Array.from(summary).slice(0, 160).join(''), session: session.id,
      created: head.ts, detail: [head.fingerprint, operation.command, operation.args, decode(match[6]), decision.reason].join(' ') };
